...
class Etri_nlp():
    def __init__(self, host_addr = "155.230.29.217", port_number = 5004, silence = False):
...
#         print(parsed_sentence)
    ...
    def make_morp_sentence(self, json_dict):
        ...
        for sentence in json_dict['sentence']:
            # get morp
            morp_elements = [morp_info['lemma']+'/'+morp_info['type']+'_' for morp_info in sentence['morp']]
            morp_element_list.extend(morp_elements)
            # save to file
        return ' '.join(morp_element_list)
    
    def get_morp_list(self, json_dict):
        morp_list=[]
        for sentence in json_dict['sentence']:
            morp_list.extend(morp_info['lemma'] for morp_info in sentence['morp'])
        return morp_list
    
    def get_ent_list(self, json_dict):
        ent_list=[]
        for ent in morphs['sentence'][0]['NE']['text']:
            ent_list.extend(ent)
        return ent_list
